...
public class GraduateBoard {
    ...
    @JsonIgnoreProperties({"hibernateLazyInitializer", "handler", "graduateBoards"}) // 순환 참조 방지 및 프론트에도 memberid값을 멤버로 보내게 하기 위해
//@JsonBackReference
    ...
    @OneToMany(mappedBy = "graduateBoard", cascade = CascadeType.ALL, orphanRemoval = true)
    //cascade = CascadeType.ALL -> 부모 자식 관계(수직관계 부모가 바뀌거나 삭제되면 자식도 영향 받음)
    // orphanRemoval -> 연관관계가 끊어지면 자식이 삭제가 됨
    @JsonManagedReference
    private List<GraduateComment> graduateComment;
