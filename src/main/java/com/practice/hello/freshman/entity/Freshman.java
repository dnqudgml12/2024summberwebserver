...
public class Freshman {
    ...
    @OneToMany(mappedBy = "freshman", cascade = CascadeType.ALL, orphanRemoval = true)
    //cascade = CascadeType.ALL -> 부모 자식 관계(수직관계 부모가 바뀌거나 삭제되면 자식도 영향 받음)
    // orphanRemoval -> 연관관계가 끊어지면 자식이 삭제가 됨
    @JsonManagedReference
    private List<com.practice.hello.freshman.entity.FreshmanComment> freshmanComment;




    @Builder //Setter역할 한다
    public Freshman(String title, String content, String author, int likes, List<FreshmanComment> freshmanComment) {
        this.title = title;
